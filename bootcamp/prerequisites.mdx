...
<Info>
  **Did you receive a `bootcamp-key.json` from the organizer?**
  - **Yes** → use the **Vertex AI** tab below (bootcamp default). The Vertex auth path lives on a separate branch.
  - **No** → use the **Google AI Studio** tab below (free-tier fallback) and follow the AI Studio sections later on this page.
</Info>

<Tabs>
  <Tab title="Vertex AI (bootcamp default) — feat/vertex-ai-auth branch">
    Use this branch if you have the organizer-provided `bootcamp-key.json`. The Vertex auth path lives on a feature branch that isn't merged into `main` yet:
  ...
  <Tab title="Google AI Studio (fallback) — main branch">
    Only use this tab if you **did not** receive a `bootcamp-key.json` from the organizer. The `main` branch supports the AI Studio `GOOGLE_API_KEY` path out of the box:
...
## API Auth: Vertex AI (Bootcamp Default)

The bootcamp organizer should have handed you a **service-account JSON file** (a `bootcamp-key.json` with `"type": "service_account"`) — you'll authenticate via **Vertex AI** using it. If you **did not** receive a JSON file, skip ahead to the [Google AI Studio fallback](#alternative-google-ai-studio-free-tier-fallback) instead.

The Strands `GeminiModel` supports both auth paths — the example code auto-switches based on environment variables, so you only need to set the env correctly.
...
## Alternative: Google AI Studio (Free Tier Fallback)

**Only use this section if you did not receive a `bootcamp-key.json` from the organizer.** Google AI Studio gives you a free `GOOGLE_API_KEY` — no billing setup required — and the examples speak it from the `main` branch without any code changes.
...
<Tabs>
  <Tab title="Vertex AI (bootcamp default)">
  ...
  <Tab title="Google AI Studio (fallback)">
    Only use this tab if you didn't receive a `bootcamp-key.json` from the organizer. Keep the `GOOGLE_API_KEY` block, paste your `AIza...` key, and leave the Vertex block commented:
...
if [ -n "$GOOGLE_APPLICATION_CREDENTIALS" ] && [ -f "$GOOGLE_APPLICATION_CREDENTIALS" ]; then
  echo "✓ Auth: GOOGLE_APPLICATION_CREDENTIALS set (Vertex AI — bootcamp default) — $GOOGLE_APPLICATION_CREDENTIALS"
elif [ -n "$GOOGLE_API_KEY" ]; then
  echo "✓ Auth: GOOGLE_API_KEY set (AI Studio fallback)"
else
  echo "✗ Auth MISSING — set GOOGLE_APPLICATION_CREDENTIALS (Vertex, default) or GOOGLE_API_KEY (AI Studio fallback)"
fi
