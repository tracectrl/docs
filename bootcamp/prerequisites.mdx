...
**Verify:**
```bash
python3 -c "import strands; print('strands', strands.__version__)"
```
...
Run this before the session. All six lines should show `✓`.
...
python3 -c "import strands" 2>/dev/null \
  && echo "✓ Strands: $(python3 -c 'import strands; print(strands.__version__)')" \
  || echo "✗ strands-agents MISSING — run: pip install strands-agents strands-agents-tools"

[ -n "$GOOGLE_API_KEY" ] \
  && echo "✓ GOOGLE_API_KEY is set" \
  || echo "✗ GOOGLE_API_KEY not set — see API Key section above"
