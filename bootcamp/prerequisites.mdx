...
### Fallback: Vertex AI Service Account (facilitator-provided)

<Note>
  **Only use this if AI Studio doesn't work for you** — corporate network blocks, region restrictions, billing-tier issues. The default path above is AI Studio.

  Ask the facilitator for `bootcamp-key.json` — a service-account credentials file for the Vertex AI project we provision for the workshop.
</Note>

**Step 1 — Save the key and point at it**

Drop `bootcamp-key.json` into your `~/Downloads/` (or anywhere stable), then export the credentials path:
...
PROJECT_ID = "internal-rt-dev-aut0"
LOCATION   = "global"
...
Expected: a short response from the model. If you hit a `PermissionDenied` or `403`, double-check the `GOOGLE_APPLICATION_CREDENTIALS` path is set in the same shell and points at a readable file.

<Warning>
  **This is a different code path than the example repo's default.** The bootcamp examples (`research_workflow_example`, `teacher_assistants_workflow_example`) wire Gemini through the AI Studio SDK (`google-genai`) via `GOOGLE_API_KEY`. If you fall back to Vertex, ask the facilitator for the Vertex-flavoured model construction snippet to drop into the example before running — it's a 3-line swap.
</Warning>

<Warning>
  **Don't commit `bootcamp-key.json` to git.** It's a real GCP service-account credential. Treat it like a password.
</Warning>
