...
<Tabs>
  <Tab title="Google AI Studio (free tier) — main branch">
    ```bash
    git clone https://github.com/tracectrl/tracectrl-bootcamp-examples.git
    cd tracectrl-bootcamp-examples
    ```
  </Tab>
  <Tab title="Vertex AI (organizer key) — feat/vertex-ai-auth branch">
    The Vertex AI auth path lives on a feature branch that isn't merged into `main` yet:

    ```bash
    git clone -b feat/vertex-ai-auth https://github.com/tracectrl/tracectrl-bootcamp-examples.git
    cd tracectrl-bootcamp-examples
    ```
    ...
    ```bash
    cd tracectrl-bootcamp-examples
    git fetch origin feat/vertex-ai-auth
    git checkout feat/vertex-ai-auth
    ```

    <Note>
      The `feat/vertex-ai-auth` branch adds env-driven Vertex support to every `GeminiModel` constructor in the examples. The `main` branch only supports the AI Studio `GOOGLE_API_KEY` path — picking `main` while configuring Vertex `.env` vars will silently fall back to AI Studio and fail with a missing-key error.
    </Note>
  </Tab>
</Tabs>
...
## Clone the Bootcamp Examples

The hands-on portions of the workshop run two pre-instrumented Strands agent examples from a separate repo. Clone it and prepare each example's `.env` now so you can jump straight to running them on the day.
...
Two example folders are inside — you'll pick which to run during the workshop:

| Example | Topology |
|---------|----------|
| `research_workflow_example/` | 3-agent chain: Researcher → Analyst → Writer |
| `teacher_assistants_workflow_example/` | Star: 1 orchestrator → 4 specialists |

Set up `.env` for **both** so either is ready when you decide.

### Configure `.env`

Both examples are self-contained: `.env` for credentials, plus two interchangeable runners — `run_agents_workflow.sh` (pip) and `run_agents_workflow_uv.sh` ([uv](https://docs.astral.sh/uv/getting-started/installation/)). Either runner creates the venv and installs everything from PyPI ([`tracectrl`](https://pypi.org/project/tracectrl/), [`tracectrl-instrumentation-strands`](https://pypi.org/project/tracectrl-instrumentation-strands/), `strands-agents[gemini]`, `strands-agents-tools`). uv is ~10× faster on the cold install.
...
<Check>
  **Checkpoint:** The `tracectrl-bootcamp-examples` repo is cloned. Both `research_workflow_example/.env` and `teacher_assistants_workflow_example/.env` exist with your `GOOGLE_API_KEY` filled in. You're ready for the bootcamp.
</Check>
