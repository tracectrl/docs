...
<Note>
  **The judge uses the same Gemini model your agents are on.** Each scaffolded guardrail passes a `GeminiModel(...)` as `judge_llm` — no extra credentials to provision. Requires `tracectrl >= 0.3.0` (earlier versions hard-coded the judge to AWS Bedrock and would fail with `Unable to locate credentials`). Run `pip show tracectrl | grep Version` to confirm.

  The Protector Plus path in Part 4 covers external LLM-firewall checks (PII, content moderation, prompt injection) where you DON'T want to roll your own judge.
</Note>

### Enable + Try It

Each example has one or two agents constructed **inside** `@tool` function bodies — that means the agent object only exists for the duration of a single tool call, and `wrap_agent_with_guardrails(agent, [...])` has nothing stable to grab. **One small refactor lifts the agent to module scope; then you uncomment two lines at the bottom of the entry file and run.**

Each scaffolded guard ships with **concrete pass/fail test prompts in its module docstring** — use one to verify the wire-in worked.

<Tabs>
  <Tab title="research_workflow_example">
    Wire in **`source_reliability_guard`** on the Writer agent:

    **Step 1 — refactor `writer_agent` to module scope.** Open `research_workflow_example/agents_workflow.py` and replace the body of `call_writer` (and lift the agent above it):

    ```python
    # Add this BLOCK above the @tool functions (after the `model = GeminiModel(...)` line):
    writer_agent = Agent(
        name="Writer",
        model=model,
        system_prompt=(
    ...
    # Simplify call_writer to use it (delete the inline Agent construction):
    @tool
    def call_writer(analysis: str) -> str:
        """Hand off analysis to the Writer Agent."""
        result = writer_agent(f"Create a report based on this analysis:\n\n{analysis}")
        return str(result)
    ```

    **Step 2 — uncomment the wire-in block** at the bottom of the same file. The block is already there, just remove the `#`:

    ```python
    from tracectrl.guardrails import wrap_agent_with_guardrails
    from guardrails import source_reliability_guard
    wrap_agent_with_guardrails(writer_agent, [source_reliability_guard])
    ```

    Leave the `fact_check_consistency_guard` line commented for now — that needs the same lift-to-scope on `analyst_agent`, one level deeper inside `call_analyst`.

    **Step 3 — run** and try a prompt that should fail (Writer produces a report with no real sources):
  ...
  <Tab title="teacher_assistants_workflow_example">
    Wire in **`pii_in_query_guard`** on the TeachAssist orchestrator. No refactor needed — `teacher_agent` is already at module scope.

    **Step 1 — uncomment the wire-in block** at the bottom of `teachers_assistant.py` (the TeachAssist line only — leave the `general_assistant_agent` line commented; that one would need a lift-to-scope refactor inside `no_expertise.py`):

    ```python
    from tracectrl.guardrails import wrap_agent_with_guardrails
    from guardrails import pii_in_query_guard
    wrap_agent_with_guardrails(teacher_agent, [pii_in_query_guard])
    ```

    **Step 2 — run** and try a prompt with PII (should fail):

    ```bash
    ./run_agents_workflow.sh
    > My name is Sarah Lee, NRIC S1234567A. Help with x^2 - 5x + 6 = 0
    > Email me the answer at student@school.edu — what's the past tense of swim?
    > I live at 12 Main Street, my name is John Doe — help me with math
    ```

    Or a clean prompt (should pass):
...
<Tip>
  **Two common pitfalls:**
  - `NameError: writer_agent is not defined` → you uncommented step 2 without doing step 1's refactor. The wire-in line needs the agent at module scope.
