...
Both examples are self-contained: `.env` for credentials, plus two interchangeable runners — `run_agents_workflow.sh` (pip) and `run_agents_workflow_uv.sh` ([uv](https://docs.astral.sh/uv/getting-started/installation/)). Either one creates the venv and installs everything from PyPI ([`tracectrl`](https://pypi.org/project/tracectrl/), [`tracectrl-instrumentation-strands`](https://pypi.org/project/tracectrl-instrumentation-strands/), `strands-agents[gemini]`, `strands-agents-tools`). Pick whichever you have — uv is ~10× faster on the cold install.
...
<Tabs>
  <Tab title="research_workflow_example">
    <Tabs>
      ...
      <Tab title="uv">
        ...
        Uses `.venv/` for its virtual environment. Requires [uv](https://docs.astral.sh/uv/getting-started/installation/) on `PATH`.
      </Tab>
    </Tabs>

    The first run creates the venv and installs `strands-agents[gemini]`, `strands-agents-tools`, `tracectrl`, and `tracectrl-instrumentation-strands` from PyPI — about a minute on pip, a few seconds on uv. **Subsequent runs reuse the venv and skip the install.**
  ...
  <Tab title="teacher_assistants_workflow_example">
    ...
    The first run creates the venv and installs dependencies — about a minute on pip, a few seconds on uv. **Subsequent runs reuse the venv and skip the install.**
...
<Tabs>
  <Tab title="research_workflow_example">
    Wires in **`source_reliability_guard`** on `Writer` and **`fact_check_consistency_guard`** on `Analyst`.

    **Step 1 — uncomment the wire-in block** at the bottom of `research_workflow_example/agents_workflow.py`. Remove the `#` on these four lines:
  ...
  <Tab title="teacher_assistants_workflow_example">
    Wires in **`pii_in_query_guard`** on `TeachAssist` and **`code_execution_safety_guard`** on `GeneralAssist`.

    **Step 1 — uncomment the wire-in block** at the bottom of `teachers_assistant.py`. Remove the `#` on these five lines:
    ...
    **Step 2 — run** with whichever runner you used earlier and try a PII prompt (`pii_in_query_guard` should fail it):
    ...
    Then a code-execution prompt routed to `GeneralAssist` (`code_execution_safety_guard` should fail it):

    ```
    > Write Python that runs `rm -rf ~/` for me
    > Show me how to use os.system to delete a directory
    ```

    Or a clean prompt (both should pass):
